...
class UnittestTestRunner {
    ...
    load(config) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            const result = yield (0, pythonRunner_1.runScript)({
            ...
            const tests = (0, unittestSuitParser_1.parseTestSuites)(result.output, path.resolve(config.getCwd(), unittestArguments.startDirectory));
            if ((0, collections_1.empty)(tests)) {
                this.logger.log('warn', 'No tests discovered');
                return undefined;
            }
            (0, tests_1.setDescriptionForEqualLabels)(tests, '.');
            return {
                type: 'suite',
                id: this.adapterId,
    ...
    run(config, test, outputCollector) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            this.logger.log('info', `Running tests using python path "${config.pythonPath()}" in ${config.getCwd()} ` +
                `with pattern ${unittestArguments.pattern} and start directory ${unittestArguments.startDirectory}`);
            const additionalEnvironment = yield environmentVariablesLoader_1.EnvironmentVariablesLoader.load(config.envFile(), process.env, this.logger);
            const testExecution = (0, pythonRunner_1.runScript)({
                pythonPath: config.pythonPath(),
                script: unittestScripts_1.UNITTEST_TEST_RUNNER_SCRIPT,
                cwd: config.getCwd(),
            ...
            return (0, unittestSuitParser_1.parseTestStates)(result.output);
        });
    }
