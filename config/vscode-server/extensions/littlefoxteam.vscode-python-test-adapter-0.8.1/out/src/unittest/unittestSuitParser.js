...
function parseTestSuites(content, cwd) {
    ...
    const allTests = (discoveryResult.tests || [])
        ...
        .filter((id) => id)
        .map((id) => id);
    const aggregatedErrors = Array.from((0, collections_1.groupBy)(discoveryResult.errors || [], (e) => e.class).entries())
    ...
    const suites = Array.from((0, collections_1.groupBy)(allTests, (t) => t.suiteId).entries()).map(([suiteId, tests]) => {
...
function parseTestStates(output) {
    const testEvents = output
        ...
        .filter((line) => line)
        .map((line) => line);
    return (0, collections_1.distinctBy)(testEvents, (e) => e.test);
}
