...
class VscodeWorkspaceConfiguration {
    ...
    collectOutputs() {
        return this.testExplorerConfiguration.get('outputs.collectOutputs', true);
    }
    showOutputsOnRun() {
        return this.testExplorerConfiguration.get('outputs.showOutputsOnRun', false);
    }
    pythonPath() {
    ...
    getConfigurationValueOrDefault(configuration, keys, defaultValue) {
        return (0, collections_1.firstNotEmpty)(keys.map((key) => () => configuration.get(key)), defaultValue);
    }
    isUnitTestEnabled() {
    ...
    isTestplanTestEnabled() {
        const overriddenTestFramework = this.testExplorerConfiguration.get('testFramework', null);
        if (overriddenTestFramework) {
            return 'testplan' === overriddenTestFramework;
        }
        return this.testExplorerConfiguration.get('testplanEnabled', false);
    }
    getTestplanPath() {
        return this.testExplorerConfiguration.get('testplanPath', 'test_plan.py');
    }
    getTestplanArguments() {
        return this.testExplorerConfiguration.get('testplanArgs', []);
    }
