...
function setDescriptionForEqualLabels(values, idSeparator) {
    const updatedLabels = mapUniqueLabelsById(values
        .filter((v) => v.id.endsWith(v.label))
...
function mapUniqueLabelsById(values, idSeparator) {
    ...
    const labelGroups = (0, collections_1.groupBy)(values, (v) => prependPrefix(v.prefix, idSeparator, v.label));
...
function extractLastElement(values) {
    if ((0, collections_1.empty)(values)) {
        return undefined;
    }
