...
function parseTestSuites(content, cwd) {
    ...
    const allTests = (discoveryResult.tests || [])
        ...
        .filter((line) => line)
        .map((line) => line);
    const suites = Array.from((0, collections_1.groupBy)(allTests, (t) => t.modulePath).entries()).map(([modulePath, tests]) => ({
        type: 'suite',
        id: modulePath,
    ...
    const aggregatedErrors = Array.from((0, collections_1.groupBy)(discoveryResult.errors || [], (e) => e.file).entries()).map(([file, messages]) => ({
...
function toTestSuites(tests) {
    ...
    const testsAndSuites = (0, collections_1.groupBy)(tests, (t) => t.idTail.includes('::'));
...
function toSuites(suites) {
    ...
    return Array.from((0, collections_1.groupBy)(suites.map((test) => splitTest(test)), (group) => group.idHead).entries()).map(([suite, suiteTests]) => ({
        type: 'suite',
        id: suite,
...
function toFirstLevelTests(tests) {
    ...
    const testsByParameterized = (0, collections_1.groupBy)(tests, (t) => t.idTail.includes('['));
    const basicTests = (testsByParameterized.get(false) || []).map(toTest);
    const parameterizedTestsBySuite = (0, collections_1.groupBy)(testsByParameterized.get(true) || [], (t) => t.idTail.substring(0, t.idTail.indexOf('[')));
    const parameterizedSuites = Array.from(parameterizedTestsBySuite.entries()).map(([baseName, parameterizedTests]) => ({
        type: 'suite',
